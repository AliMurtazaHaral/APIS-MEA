...
module.exports = {
  ...
  async  add(req, res) {
    ...
    console.log(add_event.event_id);
    // Upload media files and get URLs
    const mediaUrls = await Promise.all(
      req.files.map(async (file) => {
        const path = `${constant.AWS_S3_PROJECT_FOLDER}${constant.AWS_EVENT_FOLDER}${add_event.event_id}_${file.originalname}`;
    ...
    // Add media URLs to the media array
